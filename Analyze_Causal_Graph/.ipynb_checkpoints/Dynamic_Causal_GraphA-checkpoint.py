...
def optimal_sequence_of_interventions(exploration_sets: List, interventional_grids: Dict, initial_structural_equation_model: Callable,
                                      structural_equation_model: Callable, G: MultiDiGraph, T: int = 3, model_variables: List = None,
                                      target_variable: str = None, task: str = "min",) -> Tuple:
    """
    TensorFlow-based implementation for finding the optimal sequence of interventions.

    Args:
        exploration_sets: List of sets of variables to intervene on.
        interventional_grids: Dictionary of intervention levels for each variable set.
        initial_structural_equation_model: Initial SEM.
        structural_equation_model: Updated SEM.
        G: Graph representing causal relationships.
        T: Total time steps.
        model_variables: List of model variables.
        target_variable: The variable to optimize for.
        task: "min" for minimization, "max" for maximization.

    Returns:
        Tuple containing best intervention values, sequences, objective values, and other outputs.
    """
    assert target_variable and target_variable in (model_variables or [])

    # Initialize variables
    static_noise_model = {k: tf.zeros(T) for k in (model_variables or ["X", "Z", "Y"])}
    range_T = range(T)
    ...
    for t in range_T:
        ...
        for s in exploration_sets:
            ...
            for level in interventional_grids[s]:
                ...
                intervention_samples = sequentially_sample_model(initial_structural_equation_model, structural_equation_model, total_timesteps=T,
                                                                 interventions=intervention_blanket, sample_count=1, epsilon=static_noise_model,)
    ...
    return (best_s_values, best_s_sequence, best_objective_values, y_stars, optimal_interventions, all_CE,)
...
def create_n_dimensional_intervention_grid(limits: list, size_intervention_grid: int = 100):
    ...
    ----------
    limits : list
        Ranges for each dimension of the grid.
    size_intervention_grid : int, optional
        Number of points per dimension (default is 100).

    Returns
    -------
    tf.Tensor
        The n-dimensional intervention grid as a TensorFlow tensor.
    """
...
def get_interventional_grids(exploration_set, intervention_limits, size_intervention_grid=100) -> dict:
    ...
    exploration_set : iterable
        Sets of variables for which grids are created.
    intervention_limits : dict
    ...
    -------
    dict
        Grids indexed by exploration sets.
    """
    intervention_grid = {}

    for es in exploration_set:
        grid_size = size_intervention_grid if len(es) <= 2 or size_intervention_grid < 100 else 10

        if len(es) == 1:
            intervention_grid[es] = create_n_dimensional_intervention_grid(
                intervention_limits[es[0]], grid_size
            )
        else:
            intervention_grid[es] = create_n_dimensional_intervention_grid(
                [intervention_limits[var] for var in es], grid_size
            )
...
def make_sequential_intervention_dict(G: MultiDiGraph, T: int) -> dict:
    ...
    -------
    dict
        Dictionary of sequential interventions with None values as placeholders.
    """
...
def make_graphical_model(start_time: int, stop_time: int, topology: str, nodes: List[str], target_node: str = None, verbose: bool = False,) -> Union[MultiDiGraph, str]:
    ...
    topology : str
        "independent" or "dependent" causal topology.
    nodes : list
        List of nodes in the time-slice (e.g., ["X", "Z", "Y"]).
    target_node : str, optional
        Required for "independent" topology to specify the target node.
    verbose : bool, optional
        If True, returns DOT format of the graph for visualization.

    Returns
    -------
    Union[MultiDiGraph, str]
        DOT format of the graph or a NetworkX object.
    """
    # Validations
...
def fifteen_nodes_per_time_steps(timesteps: int = 3):
    ...
    timesteps : int, optional
        The number of time steps to include in the causal graph. Default is 3.
    ...
    tuple
        ...
        - G: Directed causal graph as a networkx DiGraph.
        ...
        - optimal_interventions: Optimal interventions sequence.
    ...
    # Initialize SEM
    sem_class = StationaryIndependentSEM() #StationaryDependentSEM()
    init_sem = sem_class.static()
    sem = sem_class.dynamic()
    ...
    # Verify intervention_domain includes all variables
    for s in exploration_sets:
        for var in s:
            if var not in intervention_domain:
                raise ValueError(f"Variable {var} is missing from intervention_domain.")
    ...
    (_, optimal_interventions, true_objective_values, _, _, all_causal_effects) = optimal_sequence_of_interventions(exploration_sets=exploration_sets,
                                                                                                                    interventional_grids=interventional_grids,
                                                                                                                    initial_structural_equation_model=init_sem,
                                                                                                                    structural_equation_model=sem,G=G,
                                                                                                                    T=timesteps,
                                                                                                                    model_variables=["X", "Z", "Y"],
                                                                                                                    target_variable="Y",)
    # Visualize the graph
    ...
    return (init_sem, sem, G, exploration_sets, intervention_domain, true_objective_values, optimal_interventions, all_causal_effects,)
...
def build_sem_hat(G: MultiDiGraph, emission_fncs: dict, transition_fncs: dict = None) -> classmethod:
    ...
    emission_fncs : dict
        Fitted emission functions (horizontal edges in the DAG).
    transition_fncs : dict
        Fitted transition functions (vertical edges in the DAG).
...
def select_sample(sample: OrderedDict, input_variables: Iterable, outside_time: int) -> np.ndarray:
    ...
    input_variables : Iterable
        Input variables for the GP regression.
    outside_time : int
        Current time index being processed.

    Returns
    -------
    np.ndarray
        Formatted input as an ndarray of shape N x D.
    """
...
def run_all_opt_models(methods_list, input_params, exploration_sets, online, use_di, transfer_hp_o, transfer_hp_i,
    ...
    ----------
    methods_list : list
        List of methods to run (e.g., "ABO", "DCBO", "BO", "CBO").
    input_params : dict
        Parameters common to all methods.
    exploration_sets : any
        Exploration sets for optimization.
    online, use_di, transfer_hp_o, transfer_hp_i, concat, estimate_sem, use_mc : bool
        Flags for specific algorithm behaviors.
    ground_truth : any
        Ground truth data.
    n_obs_t : any
        Number of observed time points.
    make_sem_estimator : callable
        Function to create SEM estimators.
    number_of_trials_BO_ABO : int
        Number of trials for BO and ABO methods.

    Returns
    -------
    models_list : list
        List of model instances for each method.
    names_list : list
