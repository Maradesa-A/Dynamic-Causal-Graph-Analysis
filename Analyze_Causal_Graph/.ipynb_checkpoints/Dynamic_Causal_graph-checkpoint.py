...
def setup_ind_scm(time_steps: int = 3):
    ...
    _, _, true_objective_values, _, _, _ = optimal_sequence_of_interventions(
        ...
        graph=dag,
        time_steps=time_steps, #time_steps=time_steps
...
def fifteen_nodes_per_time_steps(timesteps: int = 3):
    ...
    # Create base topology Directed Acyclic Graph (DAG) timesteps
    graphical_model_view = make_graphical_model(start_time=0, stop_time=timesteps - 1, topology="independent", nodes=["X", "Z", "Y"], target_node="Y", verbose=True,)
    dag = nx_agraph.from_agraph(pygraphviz.AGraph(graphical_model_view.source))
    ...
    (
        ...
        graph=dag,
        time_steps=timesteps,  #time_steps
        model_variables=["X", "Z", "Y"],
        target_variable="Y",
    )
